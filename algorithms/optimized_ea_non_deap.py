...
class PathBasedEA():
    ...
    def __init__(self, graph, robots, population_size=50, generations=50, p_cross=0.8, p_mut=0.2):
        ...
        self.conflict_count_history = []
        self.graph = graph # Use the wrapper
        self.robots = robots
    ...
    def crossover(self, parent1, parent2):
        ...
        for robot in self.robots:
            ...
            child1[robot.robot_id] = child1_path
            child2[robot.robot_id] = child2_path

        repaired_child1 = self.repair_individual(child1)
        repaired_child2 = self.repair_individual(child2)
    ...
    def evolve(self, elitism_size=5):
        ...
        for generation in range(self.generations):
            ...
            valid_fitness = [f for f in fitness_values if f != -1]
            conflict_count = sum(1 for f in fitness_values if f == -1)

            self.mean_fitness_history.append(sum(valid_fitness) / len(valid_fitness) if valid_fitness else -1)
            self.max_fitness_history.append(max(valid_fitness) if valid_fitness else -1)
            self.min_fitness_history.append(min(valid_fitness) if valid_fitness else -1)
            self.conflict_count_history.append(conflict_count)

            print(f"Generation {generation}: "
                f"Best Fitness = {best_fitness}, "
                f"Worst Fitness = {self.max_fitness_history[-1]}, "
                f"Mean = {self.mean_fitness_history[-1]:.2f}, "
            ...
            while len(next_gen) < self.population_size:
                if random.random() < self.p_crossover:
                    ...
                    children = [copy.deepcopy(self.selection(population))]
